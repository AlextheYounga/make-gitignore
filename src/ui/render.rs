...
#[expect(clippy::too_many_lines, reason = "single-pass TUI layout code is clearer in one function")]
pub fn ui(f: &mut Frame, app: &mut App) {
    ...
    // Title
    let title = Paragraph::new("Select .gitignore templates (Space to toggle, Enter to confirm, Esc to cancel)")
    ...
    // Search box
    let search_text =
        if app.search_query.is_empty() { "Type to filter...".to_string() } else { app.search_query.clone() };
    ...
    let search =
        Paragraph::new(search_text).style(search_style).block(Block::default().borders(Borders::ALL).title("Search"));
    ...
    let cursor_in_filtered = filtered_indices.iter().position(|&i| i == app.cursor_position).unwrap_or(0);
    ...
    let fetch_status = match app.fetch_status() {
        FetchStatus::Idle => String::new(),
        FetchStatus::Fetching => " | Fetching from Github...".to_string(),
        FetchStatus::Fetched => " | Fetched".to_string(),
    };
    let footer = Paragraph::new(format!("Selected: {selected_count} language(s){fetch_status}"))
